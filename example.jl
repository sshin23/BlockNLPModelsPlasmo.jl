...
@linkconstraint(graph,link1,nodes[1][:x] == nodes[2][:x] - 2)
@linkconstraint(graph,link2,nodes[1][:y] == nodes[2][:z])
@linkconstraint(graph,link3,nodes[2][:x] == nodes[3][:x][3])
@linkconstraint(graph,link4,nodes[3][:x][1] == nodes[4][:x][1])
...
ipopt(FullSpaceModel(BlockNLPModel(graph)))

    # spzeros(
    # spzeros(
    # for (idx,link) in edge.linkconstraints
